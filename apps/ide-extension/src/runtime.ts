...
import { IdeError, type IdeSettings } from "./config.js";
...
export class ExperienceRuntime {
  constructor(
    ...
    if (settings.protocol === "app-server" && appServer === undefined) {
      throw new IdeError("App Server protocol requires an App Server client");
    }
  }
  ...
  async ask(
    ...
    if (!question) throw new IdeError("Ask requires a non-empty question");
    const workspace = await this.resolveWorkspace(options.workspaceName ?? "IDE");
  ...
  async waitForRun(runId: string): Promise<{ run: Run; events: RunEvent[] }> {
    ...
    throw new IdeError(`Timed out waiting for run ${runId}`);
  }
  ...
  private async resolveWorkspace(name: string): Promise<Workspace> {
    ...
    return this.createWorkspace(name, "Obsion Experience IDE workspace");
  }
  ...
  private async resolveThread(
    ...
    if (threadId) {
      ...
      if (!match) throw new IdeError(`Thread ${threadId} was not found`);
      return match;
    }
...
function threadTitle(question: string): string {
  ...
  return first.slice(0, 80) || "IDE question";
}
