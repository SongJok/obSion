...
test("ask uses App Server for Thread and Turn mutations", async () => {
  ...
  try {
    const settings = {
      ...loadSettings({ protocol: "app-server" }, {}, "ide-token"),
      waitTimeoutMs: 1000,
      pollIntervalMs: 5,
    };
    const rest = new ObsionClient(settings.baseUrl, () => "ide-token");
    ...
    assert.equal(containsCredential(rendered, "ide-token"), false);
...
test("REST protocol creates a workspace when none exists", async () => {
  const originalFetch = globalThis.fetch;
  const calls = [];
