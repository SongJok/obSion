...
export function loadSettings(
  ...
  const baseUrl = String(
    raw.baseUrl ?? raw.base_url ?? env.OBSION_URL ?? env.OBSION_BASE_URL ?? "http://127.0.0.1:8080",
  ).replace(/\/$/, "");
  const protocol = String(raw.protocol ?? env.OBSION_PROTOCOL ?? "app-server").toLowerCase();
  if (protocol !== "rest" && protocol !== "app-server") {
    throw new DesktopError("Protocol must be rest or app-server");
  }
...
export function rejectCredentialKeys(raw: Record<string, unknown>): void {
  for (const key of Object.keys(raw)) {
    ...
    if (
      ...
      throw new DesktopError(
        "Settings must not contain credentials. Use the Desktop secret file or OBSION_TOKEN.",
      );
    }
  }
}
