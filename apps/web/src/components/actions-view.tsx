...
function DecisionModal({ approval, reason, saving, onReason, onClose, onDecision }: { approval: ActionApproval; reason: string; saving: boolean; onReason: (value: string) => void; onClose: () => void; onDecision: (approve: boolean) => void }) {
  return <div className="modal-backdrop" role="presentation"><div className="workspace-modal review-modal" role="dialog" aria-modal="true" aria-labelledby="action-decision-title"><header><span className="modal-icon"><ShieldCheck size={19} /></span><div><h2 id="action-decision-title">{approval.purpose === "EXECUTE" ? "审批动作执行" : "审批动作回滚"}</h2><p>申请人不能审批自己的动作；你的决定会绑定当前计划指纹</p></div><button className="icon-button" onClick={onClose}><X size={18} /></button></header><div className="approval-reason"><strong>申请理由</strong><p>{approval.reason}</p></div><label><span>审批意见</span><textarea autoFocus rows={4} value={reason} onChange={(event) => onReason(event.target.value)} placeholder="说明核对了哪些目标、影响和回滚条件。" /></label><footer><button className="secondary-button danger-button" onClick={() => onDecision(false)} disabled={saving || reason.trim().length < 3}>拒绝</button><button className="primary-button" onClick={() => onDecision(true)} disabled={saving || reason.trim().length < 3}>批准</button></footer></div></div>;
}

function ReasonModal({ title, body, label = "回滚原因", placeholder = "说明为什么需要撤销原动作，以及已经确认的影响。", submitLabel = "提交回滚审批", reason, saving, onReason, onClose, onSubmit }: { title: string; body: string; label?: string; placeholder?: string; submitLabel?: string; reason: string; saving: boolean; onReason: (value: string) => void; onClose: () => void; onSubmit: () => void }) { return <div className="modal-backdrop" role="presentation"><div className="workspace-modal review-modal" role="dialog" aria-modal="true" aria-labelledby="action-reason-title"><header><span className="modal-icon"><RotateCcw size={19} /></span><div><h2 id="action-reason-title">{title}</h2><p>{body}</p></div><button className="icon-button" onClick={onClose}><X size={18} /></button></header><label><span>{label}</span><textarea autoFocus rows={4} value={reason} onChange={(event) => onReason(event.target.value)} placeholder={placeholder} /></label><footer><button className="secondary-button" onClick={onClose}>取消</button><button className="primary-button" onClick={onSubmit} disabled={saving || reason.trim().length < 10}>{submitLabel}</button></footer></div></div>; }
