"""Operator and Capability ingest of DingTalk documents into Organization Knowledge."""
...
from obsion.capabilities.dingtalk_docs import (
    ...
    fetch_authorized_dingtalk_document,
    ...
    if inherit_acl:
        if not inherited:
            raise ValidationError(
                ...
                "DingTalk permission inheritance did not produce an explicit ACL",
            )
...
async def resolve_dingtalk_docs_connector(
    ...
    *,
    ...
    if connector_id is not None:
        ...
        statement = statement.where(Connector.name == "obsion-dingtalk-docs")
    connector = await session.scalar(statement)
    if connector is None or not is_dingtalk_docs_connector(connector):
        raise NotFoundError("Connector", connector_id or "obsion-dingtalk-docs")
    return connector
...
async def ingest_dingtalk_document(
    session: AsyncSession,
    principal: Principal,
    ...
    *,
...
) -> tuple[Document, DocumentVersion, int, DingTalkDocument]:
    ...
    if not is_dingtalk_docs_connector(connector):
        raise ValidationError(
    ...
    document, version, count = await service.ingest(
        session,
        principal,
        source=DINGTALK_KNOWLEDGE_SOURCE,
        ...
        extra_metadata=attach_ingest_provenance(
            ...
            provenance=VendorKnowledgeProvenance(
                ...
                sync_scope_id=fetched.workspace_id,
            ),
        ),
    )
...
async def list_dingtalk_workspaces(
    session: AsyncSession,
    principal: Principal,
    *,
    connector: Connector | None = None,
    ...
    try:
        return await client.list_workspaces(tracker=tracker)
    finally:
        await client.aclose()


async def list_dingtalk_workspace_nodes(
    session: AsyncSession,
    principal: Principal,
    workspace_id: str,
    *,
    ...
    client = _client(connector, credential, transport)
    try:
        return await client.list_workspace_nodes(workspace_id, tracker=tracker)
    finally:
        await client.aclose()


async def sync_dingtalk_workspace(
    session: AsyncSession,
    principal: Principal,
    service: KnowledgeService,
    *,
    ...
    for node in nodes:
        if node.node_type not in {"document", "doc", "file", ""}:
            skipped.append(
                {
                    "node_id": node.node_id,
                    "document_id": node.document_id,
                    "reason": "dingtalk_docs_node_type_unsupported",
        ...
        try:
            document, version, count, fetched = await ingest_dingtalk_document(
                ...
                transport=transport,
                extra_metadata={
                    "dingtalk_workspace_id": space,
                    "dingtalk_node_id": node.node_id,
                },
            )
            ingested.append(
