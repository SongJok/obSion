...
当前实现状态：IN_PROGRESS。入口执行知识答案子集；真实语义评分、Worker 执行版本证明、
...
候选的 VERIFIED 作为分数。评分必须绑定最终答案摘要。当前 CLI 使用 `PendingScorer`，
所有已完成但未独立评分的答案保持 NOT_RUN；并未配置可用于真实质量验收的评分器。
测试中使用的显式合成评分器不随 CLI 启用。
