...
新增 [ADR0142](../adr/0142-observed-worker-execution-identity.md) 的执行观察：当 `--root`
包含控制面源码时，冻结源码包的 `worker_package_sha256`。实际 Harness 在执行与每次
恢复入口追加包摘要、运行实例及初始化版本声明。验收在评分前检查所有执行观察，旧
Worker、混合版本、缺失记录或不完整事件分页均 BLOCKED。读取仍使用原 Run 与资料权限。

该摘要只测量初始化时磁盘上的 Python/JSON 包；记录始终明确 `signature_verified=false`。
不能代替不可变安装、依赖/内存测量或签名部署证明。只有数据集、没有控制面源码的目录
沿用原 Profile 格式，Worker 证明保持未完成；旧任务不补造执行事件。
