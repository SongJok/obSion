...
| `quote_reference_invalid` | 修正使用的片段标识是否属于本次服务器映射及指定Evidence，不能使用旧标识或自造标识 |
...
引用修正。ADR0137保持原候选和来源，修正改为选择服务器预先标记的连续原文片段，
程序恢复原文并执行原严格校验。需要脱敏的正文在切分前排除；标识不代表语义支持。
旧适配器返回的逐字引用仍接受原校验，不能拼接或改写引用。
